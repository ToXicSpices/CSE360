...
public class ViewNewAccount {
	...
	*/
	...
	// The only real difference is in this case we also know an email address, since it was used to send the
	// invitation to the potential user.
	/** Label displaying the main application title on the page. */
	private static Label label_ApplicationTitle = 
			new Label("Foundation Application Account Setup Page");

	/** Label displaying the "User Account Creation" header. */
	protected static Label label_NewUserCreation = new Label(" User Account Creation.");

	/** Label instructing the user to enter a username and password. */
	protected static Label label_NewUserLine = new Label("Please enter a username and a password.");

	/** TextField for entering the new user's username. */
	protected static TextField text_Username = new TextField();

	/** PasswordField for entering the new password (first entry). */
	protected static PasswordField text_Password1 = new PasswordField();

	/** PasswordField for confirming the new password (second entry). */
	protected static PasswordField text_Password2 = new PasswordField();

	/** Label used to show the password temporarily (optional). */
	protected static Label label_ShowPassword = new Label();

	/** Button that provides help for entering a username. */
	private static Button button_UsernameHelp = new Button("?");

	/** Button that provides help for entering a password. */
	private static Button button_PasswordHelp = new Button("?");

	/** Button that shows the password text when clicked. */
	private static Button button_ShowPassword = new Button("Show Password");

	/** Button that hides the password text when clicked. */
	private static Button button_HidePassword = new Button("Hide Password");

	/** Button that triggers the user account setup process. */
	protected static Button button_UserSetup = new Button("User Setup");

	// This alert is used should the invitation code be invalid
	/** Alert displayed when the invitation code entered by the user is invalid. */
	protected static Alert alertInvitationCodeIsInvalid = new Alert(AlertType.INFORMATION);

	// This alert is used should the username and passwords are invalid
	/** Alert displayed when the username or passwords are invalid. */
	protected static Alert alertUsernamePasswordError = new Alert(AlertType.INFORMATION);

	// This alert is used should show requirements for username and password
	/** Alert displayed to show username and password requirements or help. */
	...
	*/
	...
	public static void displayNewAccount(Stage ps, String ic) {
    	...
    	theRootPane.getChildren().addAll(label_NewUserCreation, label_NewUserLine, text_Username,
    			...
    			button_UsernameHelp, button_PasswordHelp, button_ShowPassword);    	
	...
	private ViewNewAccount() {
		...
		// Establish the help button for the username
		setupButtonUI(button_UsernameHelp, "Dialog", 18, 18, Pos.BASELINE_LEFT, 350, 160);
		button_UsernameHelp.setOnAction((event) -> {
			ViewNewAccount.alertUsernamePasswordHelp.setTitle("Username/Password Help");
			ViewNewAccount.alertUsernamePasswordHelp.setHeaderText("Username Requirements:");
