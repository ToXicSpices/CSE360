...
public class ControllerNewAccount {
	...
	*/
	...
	protected static void doCreateUser() {
		...
		// Make sure there's no existing same username
		if (theDatabase.getUserAccountDetails(username) == true) {
			...
			ViewNewAccount.alertUsernamePasswordError.setContentText("Username already in used");
			ViewNewAccount.alertUsernamePasswordError.showAndWait();
    		return;
    	}
		
		// Make sure the username is valid
		String usernameError = ModelNewAccount.checkValidUsername(ViewNewAccount.text_Username.getText());
		...
		String passwordError = ModelNewAccount.checkValidPassword(ViewNewAccount.text_Password1.getText());
